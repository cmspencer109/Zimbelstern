...
const int STARTING_ACCELERATION = 1000;
const int STOPPING_ACCELERATION = 750;

const int STEPS_IN_ONE_REVOLUTION = 6400; // 1600 steps * 4 pulley ratio
...
void startMotor(int speed) {
  stepper.setAcceleration(STARTING_ACCELERATION);
  ...
  stepper.move(-2147483647); // Set the largest possible position. Multiply by -1 to change direction.
  motorRunning = true;
}
...
void stopMotorOnQuarter() {
  ...
  */
  stepper.setAcceleration(STOPPING_ACCELERATION);
  stepper.stop();
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    // Serial.println(receivedByte); // TESTING ONLY, DO NOT USE IN PRODUCTION
  ...
  // Stop the motor if no bytes received for 100 ms
  if (millis() - lastByteReceivedTime > 50 && motorRunning) {
